			   ...
			   struct v4l2_crop *a)
{
	...
	int rval, org_left, left_delta, top_delta;
	...
	org_left = crop->c.left;
	while (((int)crop->c.left & 0xFFFFFFF0) != (int)crop->c.left)
		(int)crop->c.left--;

	/* Scale the change in crop_left to a change in crop_top */
	left_delta = org_left - crop->c.left;
	...
	while (((int)crop->c.width & 0xFFFFFFF0) != (int)crop->c.width)
		(int)crop->c.width--;
